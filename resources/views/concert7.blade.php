...
@section('title', (isset($event) ? $event->title : 'Ed Sheeran') . ' | + - = ÷ x Tour Jakarta')
@section('meta_description', isset($event) ? $event->description : 'Ed Sheeran Mathematics Tour Live in Jakarta')
...
@section('banner_url', (isset($event) && $event->banner_url) ? Storage::url($event->banner_url) : 'https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?auto=format&fit=crop&q=80&w=2070')
...
@section('content')
                ...
                <div class="row align-items-center justify-content-center">
                    <div class="col-xl-12">
                        <div class="slider_text text-center">
                            ...
                            <span class="wow fadeInLeft" data-wow-duration="1s" data-wow-delay=".3s">{{ isset($event) ? $event->date->format('d M, Y') : 'March 2, 2028' }}</span>
                            <h3 class="wow fadeInLeft" data-wow-duration="1s" data-wow-delay=".4s">{{ $event->title ?? '+ - = ÷ x Tour Jakarta' }}</h3>
                ...
                <div class="col-lg-8">
                    <div class="single_performer wow fadeInUp" data-wow-duration="1s" data-wow-delay=".3s">
                        ...
                        <div class="performer_heading text-center mt-4">
                            ...
                            <span style="color: #00aaff;">Global Acoustic Pop Superstar</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="about" class="about_area black_bg py-5">
        <div class="container text-center">
            <div class="section_title mb-80">
                <h3 class="wow fadeInRight text-light">{{ $event->title ?? 'Mathematics Experience' }}</h3>
                <p class="text-white opacity-75">{{ $event->description ?? 'Armed with only his loop pedal and acoustic guitar, Ed delivers a heartfelt performance.' }}</p>
            </div>
        </div>
    </div>
...
@php
    $footerDate = isset($event) ? $event->date->format('d M, Y') : 'Mar 2, 2028';
    ...
    $footerCopyright = ($event->title ?? 'Ed Sheeran') . ' | Mathematics Tour Jakarta';
@endphp